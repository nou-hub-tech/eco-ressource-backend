...
public class DatabaseSeed {
  ...
  CommandLineRunner seedAdminAndDemo() {
    return args -> {
      ...
      boolean slimAlreadyExisted = userRepository.findByEmail("slim@entreprise.tn").isPresent();
      Enterprise e = upsertEnterpriseDemo();
      if (!slimAlreadyExisted) {
        Listing l1 =
          Listing.builder()
            .enterprise(e)
      ...
      boolean karimAlreadyExisted = userRepository.findByEmail("karim@transport.tn").isPresent();
      Transporter t = upsertTransporterDemo();
      if (!karimAlreadyExisted) {
  ...
  private void upsertAdminUser() {
    User admin = userRepository.findByEmail("admin@marketplace.com").orElseGet(User::new);
  ...
  private Enterprise upsertEnterpriseDemo() {
    User enterpriseUser = userRepository.findByEmail("slim@entreprise.tn").orElseGet(User::new);
    enterpriseUser.setEmail("slim@entreprise.tn");
    enterpriseUser.setPassword(passwordEncoder.encode("demo123"));
    enterpriseUser.setFullName("Slim Ben Ali");
    enterpriseUser.setRole(Role.ROLE_ENTERPRISE);
    enterpriseUser.setEnabled(true);
    enterpriseUser.setAccountStatus(UserAccountStatus.active);
    enterpriseUser.setVerified(true);
    enterpriseUser.setPhone("+216 71 234 567");
    enterpriseUser.setCity("Tunis");
    enterpriseUser = userRepository.save(enterpriseUser);

    Enterprise enterprise =
      enterpriseRepository.findByUserId(enterpriseUser.getId()).orElseGet(Enterprise::new);
    enterprise.setUser(enterpriseUser);
    enterprise.setCompanyName("Industrie Slim SARL");
    enterprise.setSector("Metallurgy");
    enterprise.setTaxId("TN123");
    enterprise.setListingsCount(0);
    enterprise.setOrdersCount(12);
    enterprise.setRevenue("14200");
  ...
  private Transporter upsertTransporterDemo() {
    User transporterUser = userRepository.findByEmail("karim@transport.tn").orElseGet(User::new);
    transporterUser.setEmail("karim@transport.tn");
    transporterUser.setPassword(passwordEncoder.encode("demo123"));
    transporterUser.setFullName("Karim Transport");
    transporterUser.setRole(Role.ROLE_TRANSPORTER);
    transporterUser.setEnabled(true);
    transporterUser.setAccountStatus(UserAccountStatus.active);
    transporterUser.setVerified(true);
    transporterUser.setPhone("+216 72 345 678");
    transporterUser.setCity("Sfax");
    transporterUser = userRepository.save(transporterUser);

    Transporter transporter =
      transporterRepository.findByUserId(transporterUser.getId()).orElseGet(Transporter::new);
    transporter.setUser(transporterUser);
    transporter.setCompanyName("Karim Logistics");
    transporter.setSector("Transport & Logistics");
    transporter.setTaxId("TN456");
    transporter.setListingsCount(0);
    transporter.setOrdersCount(34);
    transporter.setRevenue("8750");
    transporter = transporterRepository.save(transporter);
    transporterUser.setTransporter(transporter);
    userRepository.save(transporterUser);
    return transporter;
  }
