...
import com.marketplace.backend.dto.StockItemDto;
...
public class ListingService {
  ...
  @Transactional(readOnly = true)
  public List<StockItemDto> stockMine(Authentication auth) {
    Enterprise e = requireEnterprise(auth);
    return stockItemRepository.findByEnterpriseId(e.getId()).stream()
        .map(this::toStockDto)
        .collect(Collectors.toList());
  }

  @Transactional(readOnly = true)
  public List<StockItemDto> stockAllAdmin() {
  ...
  private StockItemDto toStockDto(StockItem s) {
